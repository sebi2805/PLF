...
employee_with_good_salary([H|T], MinimumSalary, ResultList):- findall(X, (member((X, Y), [H|T]), Y >= MinimumSalary), ResultList).
...
predecessor(X,Y):- parent(X,Y), ruler(X), ruler(Y).
predecessor(X,Y):- parent(X,Z), ruler(Z), predecessor(Z,Y).
...
% unify_with_occurs_check(f(g(A),h(B,C,v)), f(g(u), h(g(w), w, g(A)))). % False
% unify_with_occurs_check(f(g(A), h(B, C, D)), f(g(u), h(g(w), w, g(A)))). % True
...
minListLength([], []). 
minListLength([H|T], [H|T]) :- not((member(X, T), length(X, L), length(H, L1), L1 < L)), minListLength(T, T).
minListLength([H|T], R) :- member(X, T), length(X, L), length(H, L1), L1 < L, minListLength(T, R).
...
palindrome(X):- string_chars(X, L), reverse(L, L).
...
employee_info_department_1_scale_2(L):- findall(X, (employee_info(X, 1, S), S > 2), L).

% b) determinati toti angajatii dintr-un anume departament 

employee_info_department(Department, L):- findall(X, employee_info(X, Department, _), L).

% c) selectati name si scale al angajatilor din departamentul 1, si scale > 3 

employee_info_department_1_scale_3(L):- findall((X, S), (employee_info(X, 1, S), S > 3), L).